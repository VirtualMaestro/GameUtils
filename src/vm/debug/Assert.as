package vm.debug
{
	...
	import vm.errors.IllegalStateError;
	...
	public class Assert
	{
		...
		{
			if (!ClassUtil.isSubclassOf(clazz, parentClass))
			{
				if (message == "" || message == null)
				{
		...
		{
			if (!ClassUtil.isImplementationOf(ClassUtil.getDefinitionForInstance(object), interfaze))
			{
				if (message == "" || message == null)
				{
