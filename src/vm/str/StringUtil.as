package vm.str
{
	import vm.errors.IllegalArgumentError;

	/**
	 * Work with string. All methods is static.
	 */
...
//				}
		...
		public static function hasText(string:String):Boolean
		{
			...
			return (StringUtil.trim(string).length > 0);
		}
		 ...
		 */
		public static function isCharWhitespace(a:String):Boolean
		{
		 ...
		 */
		public static function isCharDigit(a:String):Boolean
		{
		...
		public static function naturalCompare(a:String, b:String):int
		{
			...
			while (true)
			{
				...
				while (StringUtil.isCharWhitespace(ca) || ca == "0")
				{
					if (ca == "0")
					{
						nza++;
				...
				while (StringUtil.isCharWhitespace(cb) || cb == "0")
				{
					if (cb == "0")
					{
						nzb++;
				...
				if (StringUtil.isCharDigit(ca) && StringUtil.isCharDigit(cb))
				{
		...
		private static function compareRight(a:String, b:String):int
		{
			...
			for (; ; ia++, ib++)
			{
				...
				else if (!StringUtil.isCharDigit(ca))
				{
					return -1;
				}
				else if (!StringUtil.isCharDigit(cb))
				{
					return +1;
				}
				else if (ca < cb)
		 ...
		 *  Also method handles 'null', and in that case return empty string.
		 *  Original string isn't modified.
		 */
		public static function trim(str:String):String
		{
			...
			if (str != null && (len = str.length) > 0)
			{
				...
				if (startIndex == 0 && endIndex == (len - 1)) resultStr = str;
				else if (endIndex >= startIndex) resultStr = str.slice(startIndex, endIndex + 1);
			}
		...
		/**
		 * Method return string which have in upper case first charaster.
		 */
		public static function firstUpper(str:String):String
		{
			return (str.charAt(0).toUpperCase() + str.substring(1));
		}
	}
